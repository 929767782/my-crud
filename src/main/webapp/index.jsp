	...
	<!-- 查询、分页功能 -->
		...
		function build_emps_table(result){
			...
			$.each(emps, function(index,item){
				...
				//将以上数据拼成一行
				//append方法执行完成以后还是返回原来的元素，所以可以进行链式操作
				$("<tr></tr>").append(checkBoxTd)
					...
					.append(deptNameTd)
					.append(btnTd)
					.appendTo("#emps_table tbody"); //将数据添加到emps_table的tbody标签中
			});
		}
		...
		//因为编辑按钮是在页面加载完成事件中添加的，单击事件早于页面加载事件被绑定（注意：之前的新增按钮是页面加载完成事件之间添加的）
